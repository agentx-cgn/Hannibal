...
// H.Grids.init();                         // inits advanced map analysis
  ...
  H.Launcher.prototype.OnUpdate = function(sharedScript) {
    ...
    var 
      t0        = Date.now(),
      msgTiming = "", 
      secs      = (this.timeElapsed/1000).toFixed(1),
    ...
    // update launcher
    this.timeElapsed = this.gameState.getTimeElapsed() / 1000;

    // update context
    this.context.techtemplates     = sharedScript._techTemplates;
    this.context.player            = sharedScript.playersData[this.id];
    this.context.players           = sharedScript.playersData;
    this.context.metadata          = sharedScript._entityMetadata[this.id];
    ...
    if ((this.turn + this.player) % 8 === 5) {
      ...
      if (this.ticks === 0){
        ...
        this.timing.brn = this.bot.brain.tick(    secs, this.ticks);
        this.timing.geo = this.bot.map.tick(      secs, this.ticks);
        this.timing.gps = this.bot.groups.tick(   secs, this.ticks);
        this.timing.mil = this.bot.military.tick( secs, this.ticks);
        this.timing.eco = this.bot.economy.tick(  secs, this.ticks);
