  ...
  H.extend(H.Plugins, {
    ...
    "g.res-wood" : {
      ...
      */
      ...
      benefit:        "2 stone/sec",  // (athen) give the economy a hint what this group provides.

      position:       null,           // refers to the coords of the group's position/activities
      structure:      [],             // still unkown resource, inits at game start

      units:          ["exclusive", "wood.tree GATHEREDBY SORT > rates.wood.tree"],
      dropsite:       ["shared",    "wood ACCEPTEDBY"],
      dropsites:      ["dynamic",   "wood ACCEPTEDBY INGAME"],

      targets:        [{generic: "wood", specific: "tree"}],
      ...
      maxDropDistance: 50,            // if exceeds, order dropsite

      listener: {

        onLaunch: function(ccid, trees){

          H.Scout.updateResources(this.targets);
          trees = trees || H.Scout.nearestResource(this.position, this.targets);

          deb("     G: onlaunch %s trees: %s", this, trees);

          if (trees){
            this.trees = trees;
            this.position = trees.position;
        ...
        onInterval:  function(){
          ...
          if (this.units.count){
            ...
            if (this.units.doing("idle").count === this.units.count){
              ...
              H.Scout.updateResources(this.targets);
              this.trees = H.Scout.nearestResource(this.dropsite, this.targets);
              this.units.doing("idle").gather(this.trees);
